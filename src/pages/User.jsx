...
import { DarkModeContext } from '../context/DarkModeContext'; // ✅ Using global dark mode context

function User() {
  const { darkMode } = useContext(DarkModeContext); // 👈 using context
  ...
  const cardClass = darkMode
    ? 'bg-gray-800/80 backdrop-blur-md shadow-xl rounded-3xl p-8'
    : 'bg-white/80 backdrop-blur-md shadow-xl rounded-3xl p-8';

  const statsCardClass = darkMode
    ? 'bg-gray-700 p-5 rounded-xl shadow hover:shadow-md transition text-gray-100'
    : 'bg-yellow-100 p-5 rounded-xl shadow hover:shadow-md transition text-yellow-900';
    ...
    <div className={containerClass}>
      <div className={`max-w-4xl mx-auto ${cardClass}`}>
        ...
        <div className="flex flex-col md:flex-row items-center gap-6 mb-10">
          {/* Avatar */}
          <div className="w-32 h-32 rounded-full bg-yellow-300 flex items-center justify-center text-4xl font-bold text-white shadow-inner">
            U
          </div>
          ...
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-300">user@example.com</p>
        ...
        {/* Settings Card - Dark Mode toggle removed */}
        <div
